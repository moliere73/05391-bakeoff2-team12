...
*/
/*
// closer buttons option
void scaffoldControlLogic()
{
  float controlPadding = inchToPix(.5f); // increased padding for further distance
  
  // Adjust button positions to be slightly further from the logo square
  float offsetX = logoX;
  float offsetY = logoY;

  // Rotate counterclockwise
  text("CCW", offsetX - logoZ - controlPadding, offsetY - logoZ - controlPadding);
  if (mousePressed && dist(offsetX - logoZ - controlPadding, offsetY - logoZ - controlPadding, mouseX, mouseY) < inchToPix(.4f))
    logoRotation--;

  // Rotate clockwise
  text("CW", offsetX + logoZ + controlPadding, offsetY - logoZ - controlPadding);
  if (mousePressed && dist(offsetX + logoZ + controlPadding, offsetY - logoZ - controlPadding, mouseX, mouseY) < inchToPix(.4f))
    logoRotation++;

  // Decrease Z (size)
  text("-", offsetX - logoZ - controlPadding, offsetY + logoZ + controlPadding);
  if (mousePressed && dist(offsetX - logoZ - controlPadding, offsetY + logoZ + controlPadding, mouseX, mouseY) < inchToPix(.4f))
  ...
  // Move left
  text("left", offsetX - logoZ - controlPadding, offsetY);
  if (mousePressed && dist(offsetX - logoZ - controlPadding, offsetY, mouseX, mouseY) < inchToPix(.4f))
    logoX -= inchToPix(.02f);

  // Move right
  text("right", offsetX + logoZ + controlPadding, offsetY);
  if (mousePressed && dist(offsetX + logoZ + controlPadding, offsetY, mouseX, mouseY) < inchToPix(.4f))
    logoX += inchToPix(.02f);

  // Move up
  text("up", offsetX, offsetY - logoZ - controlPadding);
  if (mousePressed && dist(offsetX, offsetY - logoZ - controlPadding, mouseX, mouseY) < inchToPix(.4f))
    logoY -= inchToPix(.02f);

  // Move down
  text("down", offsetX, offsetY + logoZ + controlPadding);
  if (mousePressed && dist(offsetX, offsetY + logoZ + controlPadding, mouseX, mouseY) < inchToPix(.4f))
    logoY += inchToPix(.02f);
}
