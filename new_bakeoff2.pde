...
// checks if scale is right
boolean scale_Right() {
  Destination d = destinations.get(trialIndex);
  return abs(d.z - logoZ) < inchToPix(.1f);
}
  
// checks if rotation is right
boolean rotation_Right() {
  Destination d = destinations.get(trialIndex);
  return calculateDifferenceBetweenAngles(d.rotation, logoRotation) <= 5;
}
  
// checks if location is right
boolean location_Right() {
  Destination d = destinations.get(trialIndex);
  return dist(d.x, d.y, logoX, logoY) < inchToPix(.05f);
}

void displayStatus() {
  displayStatusText("Scale: ", scale_Right(), 230, 60);
  displayStatusText("Rotation: ", rotation_Right(), 230, 80);
  displayStatusText("Location: ", location_Right(), 230, 100);
}

void displayStatusText(String label, boolean isCorrect, int x, int y) {
  fill(isCorrect ? color(0, 255, 0) : color(255, 0, 0)); // Green if correct, red if incorrect
  text(label + (isCorrect ? "Correct" : "Incorrect"), x, y);
}
...
*/
void scaffoldControlLogic() {
  // Define colors for the button background and text
  int buttonFillColor = color(255, 200, 200); // Light red for better visibility
  int buttonTextColor = color(0); // Black text color

  // CCW Button (Upper left corner)
  drawButton(inchToPix(0.8f), inchToPix(0.4f), "CCW", buttonFillColor, buttonTextColor);
  if (mousePressed && dist(0, 0, mouseX, mouseY) < inchToPix(0.8f))
    logoRotation--;

  // CW Button (Upper right corner)
  drawButton(width - inchToPix(0.4f), inchToPix(0.4f), "CW", buttonFillColor, buttonTextColor);
  if (mousePressed && dist(width, 0, mouseX, mouseY) < inchToPix(0.8f))
    logoRotation++;

  // Decrease Size Button (-) (Lower left corner)
  drawButton(inchToPix(0.4f), height - inchToPix(0.4f), "-", buttonFillColor, buttonTextColor);
  if (mousePressed && dist(0, height, mouseX, mouseY) < inchToPix(0.8f))
    logoZ = constrain(logoZ - inchToPix(0.02f), .01, inchToPix(4f));

  // Increase Size Button (+) (Lower right corner)
  drawButton(width - inchToPix(0.4f), height - inchToPix(0.4f), "+", buttonFillColor, buttonTextColor);
  if (mousePressed && dist(width, height, mouseX, mouseY) < inchToPix(0.8f))
    logoZ = constrain(logoZ + inchToPix(0.02f), .01, inchToPix(4f));

  // Left Button (Left middle)
  drawButton(inchToPix(0.7f), height / 2, "Left", buttonFillColor, buttonTextColor);
  if (mousePressed && dist(0, height / 2, mouseX, mouseY) < inchToPix(0.8f))
    logoX -= inchToPix(0.02f);

  // Right Button (Right middle)
  drawButton(width - inchToPix(0.4f), height / 2, "Right", buttonFillColor, buttonTextColor);
  if (mousePressed && dist(width, height / 2, mouseX, mouseY) < inchToPix(0.8f))
    logoX += inchToPix(0.02f);

  // Up Button (Top center)
  drawButton(width / 2, inchToPix(0.4f), "Up", buttonFillColor, buttonTextColor);
  if (mousePressed && dist(width / 2, 0, mouseX, mouseY) < inchToPix(0.8f))
    logoY -= inchToPix(0.02f);

  // Down Button (Bottom center)
  drawButton(width / 2, height - inchToPix(0.4f), "Down", buttonFillColor, buttonTextColor);
  if (mousePressed && dist(width / 2, height, mouseX, mouseY) < inchToPix(0.8f))
    logoY += inchToPix(0.02f);
}

void drawButton(float x, float y, String label, int fillColor, int textColor) {
  // Draw button background
  fill(fillColor);
  stroke(0); // Black border for clarity
  strokeWeight(2);
  rectMode(CENTER);
  rect(x, y, inchToPix(0.8f), inchToPix(0.4f), 5); // Rounded corners
